...
import { moneyFormatNames, showBarNames } from '../../config';
...
const Charts: React.FC<IProps> = props => {
  ...
  // 如果选择了细分数据源时，需要对 dataTypeNames 进行特殊处理，例如从 总XXX 改为 FBAXXX
  if (dataOrigin) {
  ...
  return (
    seriesOption.some(item => item.data?.length) || loading
