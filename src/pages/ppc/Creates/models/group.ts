...
const campaign: ICreateCampaignModalType = {
  ...
  effects: {
    ...
    *getThiningBrands({ payload, resolve, reject }, { call }) {
    ...
    // 获取广告活动下拉列表
    *getCampaignList({ payload, resolve, reject }, { call }) {
      try {
        const res = yield call(getCampaignList, payload);
        resolve(res);
      } catch (err) {
        reject(err);
      }
    ...
    // 创建广告组
    *createGroup({ payload, resolve, reject }, { call }) {
      try {
